...
samples의 messages 배열에 **반드시 system 메시지를 첫 번째로 포함**하세요.
...
"samples": [
    {
        ...
        "tags": [...],
        "messages": [
            {"role": "system", "content": "시스템 프롬프트 전체 내용"},  # ← 필수!
            {"role": "user", "content": "..."},
...
- 파이프라인 결과 파일에서 대화를 가져올 때, `role: "system"` 메시지를 **절대 제거하지 마세요.**
- system 메시지가 없는 결과 파일이라면, 생성 시 사용한 시스템 프롬프트를 찾아서 messages 앞에 삽입하세요.
- 횡단차원별 1건씩, 가능하면 다양한 언어/CoT 조합으로 선택하세요.
